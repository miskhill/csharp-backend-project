...
- shows a stylish home page built with Angular Material
- calls `/api/status` and shows the live response from the C# backend
...
2. Start the frontend and confirm the home page shows the backend status.
3. Read `home.component.ts` to see how Angular loads data.
4. Read `backend-status.service.ts` to see how the HTTP call is separated from the page.
5. Change the API response in `Program.cs` and refresh the Angular page to watch the full stack update.
